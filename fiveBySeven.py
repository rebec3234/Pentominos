...
def initial_board(rows=5, cols=7):
...
def test_failure(board):
    ...
    for i in range(len(test_board)):
        for j in range(len(test_board)):
            ...
            if(test_board[i][j] == 0):
                if(place_f(test_board, [i,j]) or place_u(test_board, [i,j]) or place_p(test_board, [i,j]) or place_y(test_board, [i,j]) or place_x(test_board, [i,j]) or place_v(test_board, [i,j]) or place_n(test_board, [i,j])):
                    return False
    return True

def which_piece(board):
    pieces = ['X', 'V', 'Y', 'P', 'N', 'F', 'U']
